...
def upgrade() -> None:
    ...
    for row_id, gen_name in bind.execute(sa.text("SELECT id, generation_name FROM car_generations")).all():
        bind.execute(
    ...
    op.create_unique_constraint("uq_car_generations_car_model_id_slug", "car_generations", ["car_model_id", "slug"])
