...
content while preserving what the product parsers actually read: title tags,
og:*/twitter:* meta, product markup, and ``<script type="application/ld+json">``
structured data (used by ``extract_json_ld_product``).
"""
...
# Meta tags worth keeping — everything else is dropped. Parsers consume og:*,
# twitter:*, product:* for title/price/image, and description for fallback copy.
...
def sanitize_crawled_html(html: str) -> str:
    ...
    # Drop every <script> except JSON-LD, which product parsers read.
...
def _is_meta_allowed(meta: Tag) -> bool:
    ...
    if isinstance(http_equiv, str) and http_equiv.strip().lower() in _META_ALLOWED_HTTP_EQUIV:
        return True
    # itemprop (schema.org microdata) is load-bearing for some adapters
