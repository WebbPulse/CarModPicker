"""
Tick Performance (tickperformance.com) crawler adapter.

BigCommerce Stencil storefront (``<meta name="platform" content="bigcommerce.stencil">``)
— same theme family as ``xph.py``. No JSON-LD ``Product`` block; product-page
...
Brand: Tick shows the BigCommerce "Brand" heading on every product as
``<h2 class="productView-brand"><a><span>Brand</span></a></h2>`` — a
retailer-curated value, cleaner than a title heuristic. We use it as the
primary source and fall back to ``part_manufacturer_from_title`` /
``_from_description`` for the rare product that doesn't surface a brand row.

SKUs are retailer-prefixed (e.g. ``RUS-626030`` for a Russell hose end with
manufacturer part #626030). When MPN is populated we prefer it; otherwise we
keep the prefixed SKU as-is rather than guessing at the split — the prefix
pattern is not consistent across brands on this store.

Titles sometimes carry ``", Part #<mpn>"`` (Russell, Comp Cams, FTI) as a
trailing token; we drop that when it matches the BCData MPN so the stored
name reads cleanly across retailers.

Description: Tick's ``<article class="productView-description">`` wraps a
tabs widget — the product copy is in the ``#tab-description`` pane, not on
the article element directly (no ``itemprop="description"`` here).

Discovery: ``/xmlsitemap.php`` sitemap index → ``type=products&page=N``
children. BigCommerce product URLs live at the site root (no ``/products/``
prefix), so the filter is on the child sitemap ``type`` query, same as XPH.
Override with ``CRAWLER_TICKPERFORMANCE_START_URLS`` (comma-separated) for a
fixed list.
"""
...
TICK_BASE = "https://www.tickperformance.com"
SITEMAP_INDEX_URL = TICK_BASE + "/xmlsitemap.php"
...
# Cleanest source for SKU / price / MPN / GTIN on the Stencil theme, which
# does not emit a JSON-LD Product schema.
...
# Dedup by the last two path segments (batch + filename) after stripping the
# trailing `.W.H` dimensions, so the same photo collapses across size URLs.
_STENCIL_SIZE_SUFFIX_RE = re.compile(r"\.\d+\.\d+(\.[A-Za-z0-9]+)$")

# Titles like "... Anodized, Part #626030" — the trailing "Part #<mpn>" is
# duplicated info once we know the MPN. Stripped when the suffix matches
# the MPN (case-insensitive; surrounding whitespace / separators trimmed).
_TRAILING_PART_NUMBER_RE = re.compile(
    r"[\s,;:\-\u2013\u2014]+part\s*#?\s*([A-Za-z0-9][A-Za-z0-9._/\-]*)\s*$", re.IGNORECASE
)


def _resolve_start_urls() -> List[str]:
    """Env override wins; otherwise discover via xmlsitemap.php, then default."""
    raw = os.environ.get("CRAWLER_TICKPERFORMANCE_START_URLS", "").strip()
    if raw:
...
def _mpn_from_bcdata(attrs: Dict[str, Any]) -> Optional[str]:
    """Clean MPN if present (used for title-suffix stripping); None otherwise."""
    raw = attrs.get("mpn")
    if isinstance(raw, str) and raw.strip():
        return raw.strip()
    return None
...
def _price_cents_from_microdata(soup: BeautifulSoup) -> Optional[int]:
    """Read ``<meta itemprop="price" content="26.95">`` inside the offer scope."""
...
def _price_cents_from_og(soup: BeautifulSoup) -> Optional[int]:
    """Read ``<meta property="product:price:amount" content="26.95">``."""
...
def _extract_brand(soup: BeautifulSoup) -> Optional[str]:
    """
    Read ``<h2 class="productView-brand"><a><span>Brand</span></a></h2>``.

    Tick populates this on every product using the BigCommerce "Brand" field,
    which is retailer-curated and cleaner than a title heuristic (e.g.
    ``"UMI Performance"`` rather than the title's abbreviated ``"UMI"``).
    """
    tag = soup.find("h2", class_=re.compile(r"\bproductView-brand\b"))
    if not isinstance(tag, Tag):
        return None
    text = tag.get_text(" ", strip=True)
    return text or None
...
def _strip_trailing_part_number(name: str, mpn: Optional[str]) -> str:
    """
    Drop a trailing ``", Part #<mpn>"`` token from the title when it matches
    the BCData MPN. Keeps the ``"Part #..."`` suffix intact when the trailing
    token isn't the real MPN (rare — usually some other SKU-ish reference).
    """
    if not mpn:
        return name
    match = _TRAILING_PART_NUMBER_RE.search(name)
    if not match:
        return name
...
def _extract_description(soup: BeautifulSoup) -> Optional[str]:
    """
    Read the ``#tab-description`` pane inside the ``productView-description``
    article. Tick's template also emits a warranty/reviews pane; we target
    the description tab specifically so we don't capture warranty prose.
    """
    pane = soup.find(id="tab-description")
    ...
    article = soup.find("article", class_=re.compile(r"\bproductView-description\b"))
    if isinstance(article, Tag):
        for heading in article.find_all(["ul", "h2", "h3"]):
...
def _extract_images(soup: BeautifulSoup) -> List[str]:
    ...
    by photo identity so width variants collapse.
    """
    ordered: List[str] = []
    seen: set[str] = set()
...
class TickPerformanceAdapter(RetailerCrawlerAdapter):
    """
    Tick Performance adapter. Discovery via ``/xmlsitemap.php`` sitemap index
    (filter children to ``type=products``). Parsing uses the BCData JS blob
    (authoritative SKU / price / MPN / GTIN), a DOM sweep of the
    ``productView-images`` gallery for images, the ``productView-brand``
    block for a retailer-curated brand, and the ``#tab-description`` pane
    for the description.
    """
    ...
    def parse_product_page(self, html: str, url: str) -> Optional[ScrapedPayload]:
        """
        Parse a tickperformance.com product page into a ``ScrapedPayload``.
        Returns ``None`` when no product name can be recovered — the page
        isn't a product page (or BigCommerce is serving a soft 404).
        """
        ...
        clean_name = _strip_trailing_part_number(name, _mpn_from_bcdata(attrs))
