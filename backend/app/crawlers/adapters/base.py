...
class RetailerCrawlerAdapter(ABC):
    ...
    def extract_variants(
        ...
        ``ingest_payload`` call used for ``base_payload``. Variant ingest
        failures are logged but do not abort the page — the base part still
        lands.
    ...
    def infer_manufacturer_for_part(self, parsed: ScrapedPayload) -> Optional[str]:
        ...
        ``parsed.name`` / ``parsed.description`` for retailer-specific shape
        that doesn't fit a CSS selector (``MANUFACTURER_SELECTORS``).
