"""Service layer for parts: creation, search, filtering and purge."""
...
from app.common.api.schemas.pagination import CursorPage
from app.common.api.schemas.part import MAX_IMAGES_PER_PART, PartCreate, PartRead, PartReadWithVotes, PartUpdate
from app.common.api.services.base_dynamo_crud_service import BaseDynamoCRUDService
from app.common.api.services.page_parser import part_number_canonical
...
from app.common.api.utils.authorization import can_delete_part, require_part_edit_permission
from app.common.api.utils.response_patterns import ResponsePatterns
from app.common.db.dynamo import search
from app.common.db.dynamo import search as search_module
from app.common.db.dynamo.catalog import GTIN, MANUFACTURER_PART_NUMBER, Part
from app.common.db.dynamo.models import utc_now
from app.common.db.dynamo.tombstones import is_tombstoned
...
class PartListFilters:
    """The filters a part listing request can apply."""
    ...
    def __init__(
        ...
        *,
        ...
        """Normalise the raw query parameters into filter sets."""
    ...
    def without_cars(self) -> "PartListFilters":
        """Return a copy of these filters with the car scoping removed."""
    ...
    def has_scoping_filters(self) -> bool:
        """Report whether any filter narrows the candidate set."""
...
class PartService(BaseDynamoCRUDService[Part, PartCreate, PartUpdate]):
    """Part CRUD plus catalog search, filtering and cascade delete."""
    ...
    def get_by_id(
        ...
        """The generic CRUD fetch, with a tombstoned part treated as absent."""
        entity = super().get_by_id(entity_id, current_user, allow_public)
        if is_tombstoned(entity):
            raise self.not_found()
        return entity

    def _require(self, found: Optional[Any], entity_name: str) -> Any:
        """Return the found entity or raise 404 naming it."""
        if found is None:
            ResponsePatterns.raise_not_found(entity_name.title())
        return found

    def _validate_car_ids(self, car_ids: Iterable[UUID]) -> list[UUID]:
        """Deduplicate car ids and raise 404 when any is missing."""
    ...
    def _conflict_from_unique(self, exc: UniqueAttributeTaken, candidate: Part) -> HTTPException:
        """Build the 409 describing which unique attribute collided."""
    ...
    def create_part(
        ...
        """Create a part, rejecting a duplicate of one the user already owns."""
    ...
    def update(self, entity_id: UUID, data: PartUpdate, current_user: DBUser) -> Part:
        """Update a part, revalidating references and car links."""
    ...
    def apply_changes(self, entity: Part, **changes: Any) -> Part:
        """Persist a part with the given field changes applied."""
    ...
    def delete(self, entity_id: UUID, current_user: DBUser) -> Part:
        """Purge a part the caller is allowed to delete and return it."""
        entity = self.get_by_id(entity_id, allow_public=True)
    ...
    def purge(self, entity: Part) -> None:
        """Tombstone a part and remove everything the catalog owns that references it.

        The cross domain rows are removed by the part purge consumer that the
        tombstone triggers, not here.
        """
    ...
    def _unlink_duplicate(self, duplicate: Part) -> None:
        """Clear a duplicate's canonical pointer so it stands alone."""
    ...
    def count_by_user(self, user_id: UUID) -> int:
        """Return how many parts a user owns."""
        return self.repos.parts.count_by_user(user_id)

    def candidates(self, filters: PartListFilters) -> list[Part]:
        """Return the parts matching these filters from the narrowest index available."""
    ...
    def _matches(self, part: Part, filters: PartListFilters) -> bool:
        """Report whether a part satisfies every active filter."""
    ...
    def _sort_key(self, sort: Optional[str], parts: list[Part]) -> Callable[[Part], str]:
        """Return the sort key function for a sort option."""
        ...
        if sort in ("part_manufacturer_asc", "part_manufacturer_desc"):
            ...
            def manufacturer_key(part: Part) -> str:
                """Sort key ordering parts by manufacturer name."""
        ...
        if sort in ("category_asc", "category_desc"):
            ...
            def category_key(part: Part) -> str:
                """Sort key ordering parts by category label."""
    ...
    def list_with_votes(
        ...
        *,
        ...
        """Return one page of filtered parts with their vote tallies."""
    ...
    def with_votes(self, parts: list[Part], current_user: Optional[DBUser]) -> list[PartReadWithVotes]:
        """Attach vote tallies and the caller's own votes to parts."""
        if not parts:
            return []
    ...
    def filter_options(self, filters: PartListFilters) -> Dict[str, Any]:
        """Return the filter values still available for this result set."""
        parts = self.candidates(filters)
        result: Dict[str, Any] = {
    ...
    def _make_names(self, car_ids: list[UUID]) -> list[str]:
        """Return the distinct make names these car generations belong to."""
        if not car_ids:
            return []
    ...
    def page_by_category(self, category_id: UUID, *, limit: int, cursor: Optional[str]) -> CursorPage[PartRead]:
        """Return one page of canonical parts in a category, newest first."""
        parts = [
            part
            for part in self.repos.parts.list_by_category(category_id)
            if part.canonical_part_id is None and not is_tombstoned(part)
        ]
    ...
    def list_page_read(self, *, limit: int, cursor: Optional[str], term: Optional[str] = None) -> CursorPage[PartRead]:
        """Return one page of canonical parts, optionally search filtered."""
    ...
    def search_parts(self, term: str, *, limit: int, cursor: Optional[str]) -> CursorPage[PartRead]:
        """Return one page of parts matching a term across name and manufacturer."""
        normalized = search.normalize_term(term)
        manufacturer_ids = {
            pm.id
...
def purge_related_rows_for_parts(part_ids: Iterable[UUID]) -> None:
    ...
    A no op: the cascade is driven by the tombstone PartService.purge writes and
    performed by the part purge consumer. Kept as a reversible seam.
    """
    return
