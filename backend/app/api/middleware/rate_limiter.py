...
One layer only. Counting in the table rather than in a process is what makes the
limit real on Lambda, where every execution environment would otherwise hold its
own count. Every backend failure fails open, so an outage costs availability nothing.
