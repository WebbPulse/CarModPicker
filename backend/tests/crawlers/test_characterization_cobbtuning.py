"""SAFE-07 characterization for CobbTuningAdapter.
...
Note: COBB Tuning hydrates price and SKU client-side — price_cents is
expected to be ``null`` in the committed fixture. This is the correct
adapter behavior, not a parse bug.
"""
...
FIXTURE_DIR = Path(__file__).parent / "fixtures" / "cobbtuning"
...
def test_parse_product_page_matches_expected() -> None:
    ...
    adapter = ADAPTER_REGISTRY["cobbtuning"]()
...
#   FIXTURE = Path('tests/crawlers/fixtures/cobbtuning')
#   html = (FIXTURE/'product.html').read_text()
#   expected = json.loads((FIXTURE/'expected.json').read_text())
#   payload = CobbTuningAdapter().parse_product_page(html, expected['product_url'])
