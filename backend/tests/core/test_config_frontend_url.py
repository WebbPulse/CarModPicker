...
@pytest.mark.parametrize(
    ("frontend_url", "app_environment", "base_url", "rp_id", "origin"),
    [
        (
        ...
        ("http://localhost:4000", "development", "http://localhost:4000", "localhost", "http://localhost:4000"),
        (
...
def test_frontend_url_drives_base_url_rp_id_and_origin(
    frontend_url: str, app_environment: str, base_url: str, rp_id: str, origin: str
) -> None:
    s = _settings(APP_ENVIRONMENT=app_environment, DEBUG=False, FRONTEND_URL=frontend_url)
    assert s.frontend_base_url == base_url
    assert s.webauthn_rp_id == rp_id
    assert s.webauthn_origins_list == [origin]
