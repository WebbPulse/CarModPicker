...
from app.domains.users.consumers.user_delete import (
    MESSAGE_KIND,
    MESSAGE_VERSION,
    cascade_user_delete,
...
def stub_the_two_borrowed_helpers(request: pytest.FixtureRequest, monkeypatch: pytest.MonkeyPatch) -> Any:
    ...
    monkeypatch.setattr("app.common.api.services.part_service.PartService.purge", fake_purge, raising=True)
    monkeypatch.setattr("app.common.db.dynamo.build_lists.delete_build_list_cascade", fake_cascade, raising=True)
    return state
...
class TestAgainstRealRepositories:
    ...
    def test_the_cascade_removes_every_owned_row(self, dynamo_tables: Any) -> None:
        ...
        from app.common.db.dynamo.catalog import Category, Part
        from app.common.db.dynamo.moderation import Report, Vote
        from app.common.db.dynamo.part_price_alerts import PartPriceAlert
        from app.domains.users.consumers.delete_entrypoint import REPOSITORIES
...
class TestTheSynchronousHalfStayedBehind:
    ...
    def test_the_endpoint_still_deletes_the_user_row_itself(self) -> None:
        ...
        from app.domains.users.endpoints.users import _delete_user_everywhere
    ...
    def test_the_cascade_never_names_the_users_repository(self) -> None:
        ...
        from app.domains.users.consumers import user_delete
...
class TestEntrypoint:
    ...
    def client(monkeypatch: pytest.MonkeyPatch, repos: Any, sqs: Any) -> Any:
        ...
        from app.domains.users.consumers import delete_entrypoint as entrypoint
    ...
    def test_the_bundle_is_memoised(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("app.common.api.dependencies.repositories.build_bundle", fake_build_bundle)
