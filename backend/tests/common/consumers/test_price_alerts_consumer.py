...
from app.domains.admin.consumers.price_alerts import (
    PriceDrop,
...
def sent(monkeypatch: pytest.MonkeyPatch) -> List[Tuple[str, Any, Any, int, Any]]:
    ...
    monkeypatch.setattr("app.common.core.email.send_price_drop_alert_email", fake_send)
    return calls
...
class TestAgainstTheRealSendPath:
    ...
    def enable_ses(monkeypatch: pytest.MonkeyPatch) -> FakeSesClient:
        ...
        from app.common.core import email as email_module
    ...
    def test_the_body_carries_a_working_unsubscribe_token(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from app.common.api.dependencies.auth import decode_access_token
...
class TestEntrypoint:
    ...
    def client(monkeypatch: pytest.MonkeyPatch, repos: FakeRepos) -> Any:
        ...
        from app.domains.admin.consumers import price_alerts_entrypoint as entrypoint
