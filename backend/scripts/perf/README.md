...
`POST /api/parts/price-history` takes `require_api_key_or_admin`, so the gate
needs a credential for that half of the run. Without one the POST samples all
come back 401 and the run fails on error rate rather than latency.

The API key is the easier of the two: it needs no admin account and no login
round trip. Use the environment's `EXTENSION_API_KEY` (the `<prefix>/app`
secret's key of that name; locally, whatever you set in `backend/.env`):
...
export PERF_BEARER_TOKEN=$(curl -s -X POST http://localhost:8000/api/auth/token \
  -d "username=<admin-user>&password=<password>" | python -c 'import json,sys; print(json.load(sys.stdin)["access_token"])')
```
...
Tunable via env vars: `PERF_HOST`, `PERF_USERS`, `PERF_SPAWN_RATE`, `PERF_RUN_TIME`, `PERF_WINDOW`, `PERF_BATCH_SIZE`, `PERF_API_KEY`, `PERF_BEARER_TOKEN`.
