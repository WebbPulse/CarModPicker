...
The identity tables come from ``webbpulse.identity.create_identity_tables``, which
creates the specs ``platform-modules/aws//modules/identity`` provisions rather than
describing them again here. ``include_users`` is off because this product names its own
``users`` table in ``app.common.db.dynamo.tables``. Without the identity tables a local
stack serves the whole ``/api/auth`` surface against tables that do not exist, so no
sign-in can succeed.
...
from webbpulse.identity import create_identity_tables  # noqa: E402
...
def main() -> int:
    ...
    for name in create_identity_tables(client, settings.dynamodb_table_prefix, include_users=False):
        print(f"created {name}")
    return 0
