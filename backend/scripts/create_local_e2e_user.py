"""Seed the local stack's durable e2e user: a verified admin with a password credential.
...
Local only. It refuses to run without `DYNAMODB_ENDPOINT_URL`, the same guard
`create_dynamo_tables.py` carries, so it can never write an admin into a real account.

Usage (from backend/):
    python scripts/create_local_e2e_user.py
"""
...
def seed_user(email: str, password: str) -> str:
    ...
    `email_verified` is set because `may_sign_in` refuses an unverified address. No role is
    granted: the durable e2e user on staging is deliberately not an admin, and the suite
    asserts the 403 that rule gives on an admin only route, so seeding an admin here would
    make the local stack pass a check the deployed one fails.
    """
    import bcrypt
    ...
    if existing is None:
        ...
        user = users.update_user(existing.id, email_verified=True, is_admin=False)
...
def main() -> int:
    ...
    if not settings.DYNAMODB_ENDPOINT_URL:
        print(
            "DYNAMODB_ENDPOINT_URL is not set; refusing to seed an admin against a real AWS account.",
            file=sys.stderr,
        )
        return 1
