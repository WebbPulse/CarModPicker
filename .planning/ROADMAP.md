...
**Plans:** 6 plans
- [ ] 04-01-PLAN.md — DATA-05/07: FK index audit across 22 models + autogenerated index-only migration + pool_recycle 3600 → 1800 + regression tests for index presence and pool config (Wave 1, parallel-safe)
- [ ] 04-02-PLAN.md — DATA-08: Alembic data-migration backfill of build_logs for legacy build_lists (gen_random_uuid + WHERE NOT EXISTS idempotent; no-op downgrade with SAFE annotation) + delete lazy auto-create branches in build_logs.py :86-98 and :191-201 + orphan-guard invariant test (Wave 2)
- [ ] 04-03-PLAN.md — DATA-01/02: N+1 fix in build_logs.py via selectinload(DBBuildLogPost.author) + count query modernization + query_counter fixture using event.listen(engine, before_cursor_execute) with finally-event.remove + self-test + 2-query regression test (Wave 3)
- [ ] 04-04-PLAN.md — DATA-06: single-PR mechanical sweep of ~304 db.query / session.query / self.db.query sites across ~37 files per D-08 file-priority order (utils → services → endpoints → crawlers) + grep-based regression guard test_session_query_regression.py + OpenAPI snapshot preservation (Wave 4)
- [ ] 04-05-PLAN.md — DATA-03/04/PARTS-01: with_for_update() row-lock insertion in link_new_part/reelect_canonical/unlink_part + pytest `postgres` marker + postgres_engine fixture with PYTEST_XDIST_WORKER per-worker DB suffix + docker-compose.test.yml + backend-ci.yml postgres-tests job (postgres:16 side-car) + 10-thread ThreadPoolExecutor concurrency test asserting exactly-one-canonical/no-cycles/no-orphans invariants (Wave 5)
- [ ] 04-06-PLAN.md — DATA-09/10/PARTS-02/PARTS-03: lazy="raise" on BuildLogPost.author + BuildList.build_list_parts + BuildList.build_list_phases with callers audit + car_inference AMBIGUOUS_STANDALONE_CODES docstring + ≥20 parametrized ambiguity vectors pinning current behavior + 5 canonical-flow SQLite integration scenarios + CONVENTIONS.md "Alembic downgrade testing" subsection + test_migration_round_trip.sh reviewer-gated script (Wave 6)
